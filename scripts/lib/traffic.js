...
/** Fold an API response's daily uniques into the stored map. Returns a new map. */
function mergeTraffic(existingDaily, views) {
  const daily = Object.assign({}, existingDaily || {});
  for (const v of views || []) {
    if (!v || !v.timestamp) continue;
    const day = isoDate(v.timestamp);
    if (!/^\d{4}-\d{2}-\d{2}$/.test(day)) continue;
    daily[day] = Math.max(0, Number(v.uniques) || 0);
  }
  return daily;
}
